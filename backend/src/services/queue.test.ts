...
import { createTaskStore, enqueueProviders } from "./queue.ts";
import type { QueueDb, QueueStatement, Task } from "./queue.ts";
...
class FakeStatement implements QueueStatement {
  ...
  private execute(): Row[] {
      ...
      const picked = all
        ...
        .filter((t) => {
          const blocked = all.some(
            (o) =>
              o["status"] === "claimed" &&
              (o["lease_until"] as number) >= nowInFlight &&
              o["domain"] === t["domain"],
          );
          return !blocked;
        })
    ...
    if (q.includes("SET status = 'dlq'")) {
      ...
      let changes = 0;
      for (const task of this.db.tasks.values()) {
        if (task["status"] === "pending" && (task["attempts"] as number) >= maxAttempts && task["lease_until"] === null) {
          task["status"] = "dlq";
          changes += 1;
        }
      }
...
describe("enqueueProviders", () => {
  ...
  it("skips providers disabled or outside the window", async () => {
    ...
    const count = await enqueueProviders(db, capture.logger, [disabled as never, wrongWindow as never], "morning", "2026-08-24", NOW);
    expect(count).toBe(0);
  });
});
