...
export function renderGoogleAdsCard(
  ...
  const impEstimate = estimateDailyImpressions(ads, days, today);
  const costEstimate = estimateDailyCost(impEstimate.total, cpm);

  const statItems = [
    { label: 'Aktywne', value: fmtInt(ads.length) },
    ...(newCount > 0 ? [{ label: 'Nowe w 7 dni', value: fmtInt(newCount) }] : []),
    { label: 'Wyświetlenia (suma środków)', value: fmtInt(Math.round(impTotal)) },
    { label: 'Est. wyświetlenia/dzień', value: fmtInt(impEstimate.total) },
  ...
  return card({ title: 'Reklamy Google', body, collapsed: true });
}
