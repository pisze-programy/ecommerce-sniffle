...
describe("createStorage", () => {
  ...
  it("reads the latest snapshot", async () => {
    const db = new MockD1((query) => {
      ...
      return {
        results: [
          { snapshot_at: "2026-08-24T20:00:00.000Z", window: "evening", product_id: "p1", variant_id: "v1", quantity: 7, price: 100, regular_price: 100, available: 1 },
        ],
      };
    });
    const storage = createStorage(db, silentLogger());
  ...
  it("reads daily stats", async () => {
    const db = new MockD1((query) => {
      if (query.startsWith("SELECT * FROM daily_stats")) {
        return { results: [{ shop: "forcer.pl", day: "2026-08-24", units_sold: 7, revenue: 680, restocked: 0, sold_out_count: 0, promotion_count: 0, masked_count: 0 }] };
      }
      return { results: [] };
    });
    const storage = createStorage(db, silentLogger());
  ...
  it("reads events", async () => {
    const db = new MockD1((query) => {
      if (query.startsWith("SELECT * FROM events")) {
        return { results: [{ type: "sold", product_id: "p1", variant_id: "v1", from_quantity: 12, to_quantity: 7, from_price: 100, to_price: 100, units: 5, confidence: "exact" }] };
      }
      return { results: [] };
    });
    const storage = createStorage(db, silentLogger());
