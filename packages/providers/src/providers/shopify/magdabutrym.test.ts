...
import { extractHandle, magdabutrymModule, parseRscQuantityAvailable } from "./magdabutrym.ts";
...
describe("parseRscQuantityAvailable", () => {
  ...
  it("reads zero as sold out", () => {
    const html = '\\"id\\":\\"gid://shopify/ProductVariant/1\\",\\"title\\":\\"35\\",\\"price\\":\\"$x\\",\\"quantityAvailable\\":0';
...
describe("magdabutrymModule", () => {
  it("builds a catalog with exact stock from product pages", async () => {
    ...
    const rsc =
      ...
      '\\"id\\":\\"gid://shopify/ProductVariant/59603108856142\\",\\"title\\":\\"38\\",\\"price\\":\\"$23a\\",\\"quantityAvailable\\":4';
    const sitemap =
      '<urlset><url><loc>https://magdabutrym.com/product/pf26-dress-06-black</loc></url></urlset>';
    vi.stubGlobal(
      "fetch",
      vi.fn(async (url: unknown) => {
        const u = String(url);
        const body =
          u.includes("/sitemap.xml")
  ...
  it("skips a product page without inventory", async () => {
    ...
    expect(
      capture.records.some((record) => record.message === "magdabutrym.product no inventory"),
    ).toBe(true);
  });
});
