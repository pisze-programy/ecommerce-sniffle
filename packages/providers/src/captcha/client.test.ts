...
import { createCaptchaClient } from "./client.ts";
import type { TurnstileTask } from "./client.ts";
...
describe("createCaptchaClient", () => {
  ...
  it("logs and returns null when createTask fails", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(jsonResponse({ errorId: 15, errorDescription: "bad key" })),
    );
  ...
  it("reads the balance", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(jsonResponse({ errorId: 0, balance: 2.99 })),
    );
  ...
  it("logs when the balance call fails", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(jsonResponse({ errorId: 10, errorDescription: "no access" })),
    );
    const client = createCaptchaClient();
    const balance = await client.getBalance(capture.logger);
