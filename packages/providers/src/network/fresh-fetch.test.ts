...
import { createFreshFetch } from "./fresh-fetch.ts";
import type { Closeable, FreshFetchDeps, FreshResponse } from "./fresh-fetch.ts";
...
describe("createFreshFetch", () => {
  ...
  it("forwards the init and the agent to the underlying fetch", async () => {
    ...
    const fetchImpl = vi.mocked(deps.fetchImpl as (input: string, init: RequestInit, dispatcher: unknown) => Promise<FreshResponse>);
