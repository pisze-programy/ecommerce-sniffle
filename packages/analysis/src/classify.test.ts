...
import { computeVariantDelta } from "./classify.ts";
import type { VariantState } from "./types.ts";
...
describe("computeVariantDelta - availability", () => {
  it("returns a soldOut event for a tracked variant", () => {
    const event = computeVariantDelta(state({ quantity: 9, available: true }), state({ quantity: 0, available: false }));
...
describe("computeVariantDelta - priority and wrong input", () => {
  it("favours soldOut over a plain sold event", () => {
    const event = computeVariantDelta(state({ quantity: 2, available: true }), state({ quantity: 0, available: false }));
    expect(event?.type).toBe("soldOut");
  });
