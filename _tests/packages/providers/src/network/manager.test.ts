...
import {
  ...
  requestBodyBytes,
  responseBodyBytes,
} from '../../../../../packages/providers/src/network/manager.ts';
...
describe('measureFetch', () => {
  ...
  it('counts the body bytes when the response has no content-length', async () => {
    const capture = captureLogger();
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
    ...
    const measured = measureFetch(fetchFn, capture.logger, 'wkdzik', 'proxy');
    const result = await measured('https://x.pl/basket/');
    expect(capture.records).toHaveLength(0);
    expect(await result.text()).toBe('hello world');
    expect(capture.records).toHaveLength(1);
    const context = capture.records[0]?.context;
    expect(context?.['responseBytes']).toBe(11);
    expect(context?.['status']).toBe(200);
  });

  it('counts the arrayBuffer bytes when the response has no content-length', async () => {
    const capture = captureLogger();
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
    ...
    expect(capture.records[0]?.context?.['responseBytes']).toBe(5);
  });

  it('logs when a chunked body is cancelled', async () => {
    const capture = captureLogger();
    const fetchFn = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
  ...
  it('preserves status, ok and headers on a class-like chunked response', async () => {
    const capture = captureLogger();
    class FakeResponse {
      get ok(): boolean {
        return true;
    ...
    const fetchFn = vi.fn().mockResolvedValue(new FakeResponse());
    const measured = measureFetch(fetchFn, capture.logger, 'wkdzik', 'proxy');
    const result = await measured('https://x.pl/');
    ...
    expect(await result.text()).toBe('hello world');
    expect(capture.records[0]?.context?.['status']).toBe(200);
  });
});
