...
import {
  buildEmbeddedQtyProvider,
  fetchMagentoCookie,
  parseCurrency,
  parseEmbeddedQty,
} from '../../../../../../packages/providers/src/providers/magento/embedded-qty.ts';
...
const PAGE_HTML =
  ...
  blockHtml('29.99', '17959', '"18000":{"182":"5"}', '"18000":"BRMK001"') +
  `</html>`;
...
describe('parseEmbeddedQty', () => {
  ...
  it('skips cards without a matching block', () => {
    const html = cardHtml('1', 'https://x.pl/a', 'A') + blockHtml('10', '2', '"3":{"182":"1"}', '"3":"SKU3"');
...
describe('fetchMagentoCookie', () => {
  it('joins every set-cookie header and cancels the body', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ headers, body: { cancel } }));
...
describe('buildEmbeddedQtyProvider', () => {
  ...
  it('builds a catalog from the nav, brands and pagination', async () => {
    ...
    undiciFetchMock.mockImplementation(directFetch);
    const provider = buildEmbeddedQtyProvider(config, logger);
  ...
  it('measures every catalog request in the proxy.request log', async () => {
    const records: { level: string; message: string; context: Record<string, string | number | boolean | null> }[] = [];
    ...
    const measured = records.filter((record) => record.message === 'proxy.request');
    const urls = measured.map((record) => record.context['url']);
    expect(urls).toContain('https://influcenter.pl/gora');
    expect(urls).toContain('https://influcenter.pl/gora?p=2');
    expect(urls).toContain('https://influcenter.pl/marki');
    expect(urls).toContain('https://influcenter.pl/marki/ekipa');
    for (const record of measured) {
      expect(typeof record.context['responseBytes']).toBe('number');
      expect(Number(record.context['responseBytes'])).toBeGreaterThan(0);
      expect(record.context['status']).toBe(200);
      expect(record.context['via']).toBe('proxy');
    }
  });

  it('logs a failed request with status null and zero bytes', async () => {
    const records: { level: string; message: string; context: Record<string, string | number | boolean | null> }[] = [];
    const logger = createLogger((record) => records.push(record));
    vi.stubGlobal(
      'fetch',
    ...
    const provider = buildEmbeddedQtyProvider(config, logger);
    await expect(provider.fetchCatalog()).rejects.toThrow();
    const failed = records.filter((record) => record.message === 'proxy.request' && record.context['status'] === null);
    expect(failed.length).toBeGreaterThan(0);
    expect(failed[0]?.context['responseBytes']).toBe(0);
    expect(String(failed[0]?.context['error'])).toContain('network down');
  });
  ...
  it('rotates the cookie on a persistent 429', async () => {
    ...
    const provider = buildEmbeddedQtyProvider(config, logger);
    const catalog = await provider.fetchCatalog();
    const cookieCalls = undiciFetchMock.mock.calls.filter((call) => String(call[0]).endsWith('/'));
