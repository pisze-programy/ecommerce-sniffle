...
import {
  ...
  applyOutcome,
  probeVariantStock,
} from '../../../../../../packages/providers/src/providers/shopify/implementations/cart-probe.ts';
import { RateLimiter } from '../../../../../../packages/providers/src/network/limiter.ts';
import type { Variant } from '../../../../../../packages/providers/src/types.ts';
...
describe('parseChangeResponse', () => {
  ...
  it('reads a clamped quantity in the pozycja format', () => {
    const capture = capturingLogger();
    expect(parseChangeResponse('{"message":"Tylko 1 pozycja"}', capture.logger)).toEqual({
      quantity: 1,
      available: true,
    });
  });
...
describe('probeVariantStock', () => {
  it('logs a warning when the cart add fails', async () => {
    const capture = capturingLogger();
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse(422, '{"message":"error"}')));
    const outcome = await probeVariantStock(
      'wakenbake.pl',
      '54030862188885',
      capture.logger,
      undefined,
  ...
  it('blocks and logs when the cart add hits a cloudflare challenge', async () => {
    const capture = capturingLogger();
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse(429, '<title>Verifying your connection...</title>')));
    const outcome = await probeVariantStock('booso.pl', '1', capture.logger, undefined, new RateLimiter(1000), 1);
    expect(outcome).toEqual({ quantity: null, available: null });
    expect(capture.records.some((record) => record.message === 'cartprobe.challenge blocked')).toBe(true);
  });
  ...
  it('retries a blocked add once and succeeds on the second attempt', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce(jsonResponse(200, '{"id":1}', 'cart=abc'))
        .mockResolvedValueOnce(jsonResponse(200, '{"items":[{"quantity":7}],"item_count":1}'))
    );
    const outcome = await probeVariantStock('booso.pl', '1', capture.logger, undefined, new RateLimiter(1000), 1);
    expect(outcome).toEqual({ quantity: 7, available: true });
    expect(capture.records.some((record) => record.message === 'cartprobe.retry')).toBe(true);
  ...
  it('retries a blocked change and succeeds on the second attempt', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValueOnce(jsonResponse(429, 'blocked'))
        .mockResolvedValueOnce(jsonResponse(200, '{"id":1}', 'cart=abc'))
        .mockResolvedValueOnce(jsonResponse(200, '{"items":[{"quantity":3}],"item_count":1}'))
    );
    const outcome = await probeVariantStock('booso.pl', '1', capture.logger, undefined, new RateLimiter(1000), 1);
