...
import { ALL_MODULES, buildProvider, buildStockRevealer, createLogger } from '@ecommerce-sniffle/providers';
import type { Logger, LogRecord, ProviderConfig, ProviderModule } from '@ecommerce-sniffle/providers';
import { isStockRevealer, runVpsPass } from '../../../orchestrator/src/runner.ts';
import type { VpsPassOptions } from '../../../orchestrator/src/runner.ts';
...
beforeEach(() => {
  ...
  undiciFetchMock.mockRejectedValue(new Error('network disabled in runner tests'));
});
...
describe('runVpsPass', () => {
  ...
  it('collects all not-implemented providers as failures', async () => {
    const result = await runVpsPass(silentLogger(), { directFetch: networkDisabledFetch() });
    expect(result.failed).toHaveLength(31);
  });
  ...
  it('does not send when the ingest env is missing', async () => {
    ...
    expect(
      capture.records.some((record) => record.message === 'ingest disabled: BACKEND_URL or INGEST_SECRET not set')
    ).toBe(true);
  });
