...
import { runExecutorPass } from '../../../orchestrator/src/executor.ts';
import type { QueueClient, Task } from '../../../orchestrator/src/queue-client.ts';
...
describe('runExecutorPass', () => {
  ...
  it('stops when memory is low', async () => {
    ...
    expect(result.processed).toBe(0);
    expect(capture.records.some((record) => record.message === 'memory low, stop executor')).toBe(true);
  });
