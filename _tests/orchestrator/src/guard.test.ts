...
import { acquireLock, enoughMemory, releaseLock } from '../../../orchestrator/src/guard.ts';
...
describe('lock', () => {
  ...
  it('treats an unreadable lock as stale and logs warnings', () => {
    ...
    expect(capture.records.some((record) => record.message === 'lock open failed')).toBe(true);
    expect(capture.records.some((record) => record.message === 'lock read failed, treat as stale')).toBe(true);
    expect(capture.records.some((record) => record.message === 'stale lock unlink failed')).toBe(true);
  });
  ...
  it('logs a warning when a live lock is held', () => {
    ...
    expect(acquireLock(capture.logger, path)).toBe(false);
    expect(capture.records.some((record) => record.message === 'lock is held by another run')).toBe(true);
  ...
  it('logs a debug record when releasing a missing lock', () => {
    ...
    expect(capture.records.some((record) => record.message === 'lock release failed')).toBe(true);
  });
});
