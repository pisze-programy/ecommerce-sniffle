...
import { cronReport, sendReport, taskReport } from "./snitch.ts";
...
describe("sendReport", () => {
  it("sends a report with the token, source and notify", async () => {
    ...
    await sendReport(
      { source: "ecommerce-pulse/vps/godsavequeens", status: "failed", data: { webshareBytes: 100 }, notify: "on-error" },
      capture.logger,
    );
  ...
  it("warns when the snitch is not configured", async () => {
    ...
    expect(
      capture.records.some((record) => record.message === "snitch not configured"),
    ).toBe(true);
  });
  ...
  it("warns when the report is rejected", async () => {
    ...
    expect(
      capture.records.some((record) => record.message === "snitch report failed"),
    ).toBe(true);
  });
});
